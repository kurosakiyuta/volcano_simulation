...
void rk4c(double y[], double dydx[], int nc,int n, double x, double h,double P[], double yout[],double  watersuminitial,
	void(*devris)(double,double[], double[], double[],double)) { //devris=(x,y,dydx)
	int i;
	...
	(*devris)(xh,P, yt, dyt, watersuminitial);

	for (i = 0; i <= nc-1; i++) {
		yt[i] = y[i] + hh * dyt[i];
	}

	(*devris)(xh, P, yt, dym, watersuminitial);
	...
	(*devris)(x + h, P, yt, dyt, watersuminitial);
